...
export class VehicleCustomization {
  ...
  /**
   * Adiciona uma imagem de logo por URL (sem precisar de File object)
   * @param {string} url - URL da imagem (pode ser relativa ou absoluta)
   * @param {string} name - Nome exibido na UI
   * @returns {{id: string, name: string, url: string}}
   */
  addLogoImageByUrl(url, name) {
    const item = {
      id: crypto.randomUUID?.() || String(Date.now() + Math.random()),
      name,
      url,
    };
    this.imagePool.items.push(item);
    this.onRenderRequest();
    return item;
  }
